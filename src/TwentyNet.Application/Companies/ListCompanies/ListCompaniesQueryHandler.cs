...
public sealed class ListCompaniesQueryHandler : IRequestHandler<ListCompaniesQuery, PagedResult<CompanyDto>>
{
    ...
    public async Task<PagedResult<CompanyDto>> Handle(ListCompaniesQuery request, CancellationToken cancellationToken)
    {
        ...
        var query = _companyRepository.AsQueryable().Where(c => c.WorkspaceId == workspaceId);
        ...
        var totalCount = await _companyRepository.CountAsync(query, cancellationToken);

        query = DynamicSorter.ApplySorts(query, sorts);

        var items = await _companyRepository.ToListAsync(
            query.Skip(request.Skip).Take(request.Take),
            cancellationToken);

        var dtos = _mapper.Map<IReadOnlyList<CompanyDto>>(items);
        return new PagedResult<CompanyDto>(dtos, totalCount, request.Skip, request.Take);
    }
}
